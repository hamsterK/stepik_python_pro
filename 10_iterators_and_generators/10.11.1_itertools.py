...
соответствующий рост. Группы должны быть расположены в порядке увеличения роста, каждая на отдельной строке,
имена в группах — в алфавитном порядке.
"""
...
persons.sort(key=lambda p: (p.height, p.name))
groups = groupby(persons, key=lambda p: p.height)

for key, person in groups:
    print(f"{key}: {', '.join(p.name for p in person)}")
