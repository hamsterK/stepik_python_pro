...
plan_sort = {'Gold': 0, 'Silver': 1, 'Bronze': 2, 'Basic': 3}
...
for user in sorted(users, key=lambda x: (plan_sort[x.plan], x.email)):
    temp_dict = dict()
    for field, value in zip(User._fields, user):
        temp_dict[field] = value
    print(f'{temp_dict["name"]} {temp_dict["surname"]}')
    print(f'  Email: {temp_dict["email"]}')
    print(f'  Plan: {temp_dict["plan"]}\n')
