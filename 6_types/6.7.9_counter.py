...
Например, для группы {is, to, hi, no} это числа 2 и 4. Программа должна вывести данные о каждой группе, расположив их
в порядке увеличения количества слов в них, каждые на отдельной строке, в следующем формате:
...
len_words = Counter([len(i) for i in input().split()])
[print(f"Слов длины {k}: {len_words[k]}") for k in (sorted(len_words, key=lambda x: len_words[x]))]
